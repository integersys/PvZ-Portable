...
void CutScene::PlaceAZombie(ZombieType theZombieType, int theGridX, int theGridY)
{
	...
	PVZP_ASSERT(aZombie);
	aZombie->mPosX = theGridX * 56 + 830 + BOARD_ADDITIONAL_WIDTH * 2;
	...
	if (theZombieType == ZombieType::ZOMBIE_BUNGEE)
	{
		...
		aZombie->mRow = 0;
		aZombie->mPosX = theGridX * 50.0f + 950.0f + BOARD_ADDITIONAL_WIDTH * 2;
		aZombie->mPosY = 50.0f;
	}
	else if (theZombieType == ZombieType::ZOMBIE_BOBSLED)
	{
		...
		aZombie->mRow = 0;
		aZombie->mPosX = 1105.0f + BOARD_ADDITIONAL_WIDTH * 2;
